...
auth('13556534179', '6PpiKmr5dyYc')  # 账号是申请时所填写的手机号；密码为聚宽官网登录密码
...
#全局变量
data_root = 'E:/pythonProject/DeltaTrader/data/'
...
def get_index_list(index_symbol='000300.XSHG'):
    """
    获取指数成分股 https://www.joinquant.com/indexData
...
def export_data(data, filename, type, mode=None):
    ...
    if mode == 'a':
        ...
        data.to_csv(file_root,index=False) # 重新写入,不新增列名

        #print(data)
    else:
        data.to_csv(file_root)  # 判断一下file是否存在 > 存在:追加 / 不存在:保持

    print('以成功存储至:', file_root)


def get_csv_price(code,strat_date,end_date,columns=None):
    """
    获取本地数据，且顺便完成数据更新工作
    :param code: str,股票代码
    :param strat_date: str,起始日期
    ...
    # print(data)
    # 根据日期筛选股票数据
    return data[(data.index >= strat_date) & (data.index <= end_date)]

def transfer_price_freq(data, time_freq):
    """
    将数据转换为制定周期:开盘价 （周期第1天）、收盘价 （周期最后1天）、最高价(周期内)、最低价(周期)
    :param data:
    :param time_freq:
    :return:
    """
...
def get_single_valuation(code, date, statDate):
    ...
    :return:
    """
    data = get_fundamentals(query(indicator).filter(valuation.code == code), date=date, statDate=statDate)  # 获取财务指标数据
    return data                                                                  # 实时更新数据: 假设每天更新日K数据  >  存到csv文件里面 > data.to_csv(append)
...
def update_daily_price(stock_code, type='price'):
    ...
    if os.path.exists(file_root): # 如果存在对应文件
        ...
        startdate = pd.read_csv(file_root,usecols=['date'])['date'].iloc[-1] #获取股票最后一个日期
        # print(startdate)
        df = get_single_price(stock_code,'daily',startdate,datetime.date.today())
        # 3.3追加到已有文件中
        export_data(df, stock_code, 'price', mode='a')
    else:
        # 重新获取该股票行情数据
